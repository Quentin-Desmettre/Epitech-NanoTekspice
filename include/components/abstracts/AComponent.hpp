...
namespace nts
{
    ...
    class AComponent : public nts::IComponent {
        ...
        protected:
            template <size_t T3>
            static void getPossibilites(std::set<int> &result, const std::array<std::vector<int>, T3> &combins, size_t depth = 0, int carryOut = 0)
            {
                if (depth + 1 == combins.size()) {
                    for (std::size_t i = 0; i < combins[depth].size(); i++)
                        result.insert(combins[depth][i] * (1 << depth) + carryOut);
                    return;
                }
                for (std::size_t i = 0; i < combins[depth].size(); i++)
                    getPossibilites(result, combins, depth + 1, carryOut + combins[depth][i] * (1 << depth));
            }
